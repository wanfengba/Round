...
{
    if (_checkXSS($text)) {
        echo "<div class=\"secret\">此条为悄悄话，首页不可见</div>";
    } else {
        $text = strip_tags($text);
        $text = _parseReply($text);
        echo preg_replace('/\[secret\](.*?)\[\/secret\]/sm', '<div class=\"secret\">此条为悄悄话，首页不可见</div>', $text);
    }
}
/* 过滤评论回复 */
...
function _parseReply($text)
{
    $text = preg_replace_callback(
        ...
        function ($match) {
            return '<img class="owo_image lazyload" src="' . Helper::options()->themeUrl . '/assets/owo/paopao/' . str_replace('%', '', urlencode($match[1])) . '_2x.png" alt="表情"/ no-view>';
        },
        $text
    );
    $text = preg_replace_callback(
        ...
        function ($match) {
            return '<img class="owo_image lazyload" src="' . Helper::options()->themeUrl . '/assets/owo/aru/' . str_replace('%', '', urlencode($match[1])) . '_2x.png" alt="表情"/ no-view>';
        },
        $text
    );
    return $text;
